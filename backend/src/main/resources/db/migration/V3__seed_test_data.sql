do $$
declare
    seed_enabled boolean := coalesce(current_setting('schooldays.seed_test_data', true), 'false') = 'true';
begin
    -- Development seed data for local/manual testing.
    -- Disabled by default. Enable with PostgreSQL option:
    -- options=-c schooldays.seed_test_data=true
    -- Test password for both users: SchoolDays123!
    if not seed_enabled then
        return;
    end if;
