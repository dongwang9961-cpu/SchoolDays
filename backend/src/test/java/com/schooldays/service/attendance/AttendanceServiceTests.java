...
class AttendanceServiceTests {
    ...
    @Test
    void parentCheckInWindowUsesClassSiteTimezone() {
        dsl.update(CLASSES)
                .set(CLASSES.START_DATE, LocalDate.parse("2026-06-01"))
                .set(CLASSES.METADATA, JSONB.valueOf("""
                        {
                          "classType": "weekly",
                          "weekdays": ["TUESDAY"],
                ...
                .execute();
        Clock utcBoundaryClock = Clock.fixed(
                Instant.parse("2026-07-02T03:30:00Z"),
                ZoneId.of("UTC")
        );
        AttendanceService boundaryService = new AttendanceService(new AttendanceDao(dsl), utcBoundaryClock);

        var response = boundaryService.parentCheckIn(
                parentUserId,
                new AttendanceCheckInRequest(tenantId, childId, classId, LocalDate.parse("2026-06-30"))
        );

        assertThat(response.classDate()).isEqualTo(LocalDate.parse("2026-06-30"));
        assertThat(response.status()).isEqualTo("checked_in");
    }
}
