...
4. For email link registration, system sends a registration link to the parent email address from a verified platform sender such as `noreply@schooldays.cc`, with the school name used as the display name.
...
- System-sent registration and invitation links should use a platform-controlled sender such as `noreply@schooldays.cc`, not a school administrator's Gmail account. Use the school name as the email display name so one verified email domain can support many schools.
...
- Registration-link email is system mail. The default sender is `noreply@schooldays.cc`; for example, Longlong Art Studio can send as `Longlong Art Studio <noreply@schooldays.cc>`.
